...
const FAQS: FAQItem[] = [
  {
    ...
    answer: 'OpenComm is a professional marketplace and discovery platform that connects verified professionals, job seekers, and clients directly in a transparent ecosystem.'
  },
  {
    id: 'faq-2',
    question: 'Is OpenComm free to use?',
    answer: 'Yes! Creating an account, browsing jobs, discovering professionals, and building your basic profile on OpenComm is free.'
  },
  {
    id: 'faq-3',
    question: 'Who can create an account?',
    answer: 'Anyone looking to discover jobs, offer professional services, build a career portfolio, or hire skilled individuals can create an OpenComm account.'
  },
  {
    id: 'faq-4',
...
export default function AboutPage({ isLoggedIn = false, onOpenAuth }: AboutPageProps) {
  ...
  return (
    <div className="w-full text-left font-sans max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-12 pb-[calc(110px+env(safe-area-inset-bottom))]">
      ...
      {/* D. Why OpenComm Exists (Core Purpose) */}
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 space-y-2">
            ...
            <p className="text-xs text-slate-600 dark:text-zinc-400 leading-relaxed">
              Many talented individuals struggle to present their capabilities effectively to clients without paying excessive upfront listing fees.
            </p>
          </div>
          <div className="p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 space-y-2">
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">Clients Struggle to Find Talent</h3>
            <p className="text-xs text-slate-600 dark:text-zinc-400 leading-relaxed">
              Finding verified contractors or specialized professionals requires navigating fragmented directories and unverified social groups.
            </p>
          </div>
          <div className="p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 space-y-2">
          ...
          <div className="p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 space-y-2">
            ...
            <p className="text-xs text-slate-600 dark:text-zinc-400 leading-relaxed">
              OpenComm brings professional profiles, job discovery, direct search, and secure communication together in one unified platform.
            </p>
          </div>
        </div>
      </section>

      {/* E. Who OpenComm Is For */}
      <section className="space-y-4">
        <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {[
            { title: 'Job Seekers', desc: 'Discover verified jobs and projects.' },
            { title: 'Freelancers', desc: 'Showcase portfolios and connect with clients.' },
            { title: 'Skilled Workers', desc: 'Highlight technical & practical trades.' },
            { title: 'Local Professionals', desc: 'Offer on-demand services nearby.' },
            { title: 'Students & Entry Level', desc: 'Build initial experience & credentials.' },
            { title: 'Employers & Clients', desc: 'Post opportunities & hire workers.' },
            { title: 'Companies & Teams', desc: 'Corporate organization management.', comingSoon: true }
          ].map((user, idx) => (
      ...
      {/* L. Final CTA */}
      <section className="p-8 rounded-3xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-center space-y-4 shadow-xl">
        <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight">Explore OpenComm</h2>
        <p className="text-xs sm:text-sm max-w-xl mx-auto text-blue-100 font-medium">
          Start browsing verified work opportunities or discover skilled professionals today.
        </p>
        <div className="flex flex-wrap justify-center items-center gap-3 pt-2">
          ...
          <button
            ...
            className="px-6 h-11 rounded-xl text-xs sm:text-sm font-bold text-white bg-indigo-900/50 hover:bg-indigo-900/70 border border-white/20 shadow-md active:scale-98 transition-all cursor-pointer"
          >
            Find Professionals
          </button>
          {!isLoggedIn && onOpenAuth && (
            <button
              ...
              className="px-6 h-11 rounded-xl text-xs sm:text-sm font-bold text-white bg-black/30 hover:bg-black/40 border border-white/20 shadow-md active:scale-98 transition-all cursor-pointer"
            >
              Create Account
            </button>
          )}
