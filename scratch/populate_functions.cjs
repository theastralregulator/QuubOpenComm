...
const functionsDir = path.join('c:\\Users\\HP\\Downloads\\QuubOpenComm\\QuubOpenCommV2', 'supabase', 'functions');
...
const templateCode = `import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
...
for (const fn of functions) {
  ...
  if (fs.existsSync(fnPath)) {
    fs.writeFileSync(fnPath, templateCode, 'utf8');
  }
}

console.log('Successfully updated generated Supabase Edge Functions with boilerplate.');
