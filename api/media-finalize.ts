...
// Consolidated serverless function for media finalization:
// - /api/media-finalize (Permanent Chat)
// - /api/negotiation-media-finalize (Negotiation Chat V2)
export default async function handler(req: any, res: any) {
  ...
  if (isNegotiation) {
    // ==================================================
    // NEGOTIATION CHAT V2 MEDIA FINALIZATION
    // ==================================================
    ...
    // ==================================================
    // PERMANENT CHAT MEDIA FINALIZATION
    // ==================================================
    const authUser = await verifyUserAuth(req);
    if (!authUser) {
      return res.status(401).json({ error: 'Authentication required' });
    ...
    // Document Security Verification via Scanner
    if (mediaType === 'document') {
      const maxDocBytes = 20 * 1024 * 1024;
      if (fileSizeBytes > maxDocBytes) {
        await resetIntentLeaseToPending();
